          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground font-mono">
                {medical?.blood_type || "—"} • {profile?.date_of_birth ? `${new Date().getFullYear() - new Date(profile.date_of_birth).getFullYear()} years` : "—"} • Health ID: {profile?.health_id || "—"}
              </div>
            </div>
          </div>
          {allergies.length > 0 && (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-lg bg-muted/40 p-3">
                ...
                <div className="text-sm font-medium text-foreground">
                  {med.start_date ? format(new Date(med.start_date), "MMM d, yyyy") : "—"}
                </div>
              </div>
              <div className="rounded-lg bg-muted/40 p-3">
                <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1 flex items-center gap-1">
                ...
                <div className="text-sm font-medium text-foreground">
                  {med.end_date ? format(new Date(med.end_date), "MMM d, yyyy") : "Ongoing"}
                </div>
              </div>
            </div>
