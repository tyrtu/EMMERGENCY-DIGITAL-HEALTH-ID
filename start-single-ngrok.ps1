...
try {
    # Launch ngrok in its own persistent PowerShell window so the tunnel
    # remains open until you close it. Capture the process so we can
    # stop it cleanly in the cleanup action.
    Write-Host "Launching ngrok in a new window..." -ForegroundColor Cyan
    $ngrokProcess = Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$scriptRoot'; ngrok http 5173 --log=stdout" -WindowStyle Normal -PassThru

    try {
        # Wait for the ngrok window/process to exit (this keeps the script alive)
