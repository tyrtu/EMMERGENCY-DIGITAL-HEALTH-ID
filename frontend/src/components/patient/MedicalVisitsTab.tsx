import { Calendar, Stethoscope, FileText, Clock, Pill } from "lucide-react";
...
export default function MedicalVisitsTab() {
  ...
  const [expandedVisitNotes, setExpandedVisitNotes] = useState<Record<string, boolean>>({});
    ...
    <div className="space-y-6">
      ...
      {visits && visits.length > 0 ? (
        visits.map((visit) => {
          ...
          const hasLongNotes = rawNotes.length > 140;
          const isExpanded = Boolean(expandedVisitNotes[visit.id]);
            ...
            <div key={visit.id} className="card-medical space-y-4">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-sm font-semibold text-foreground">{visit.diagnosis || visit.department || "General Consultation"}</div>
                    <div className="text-xs text-muted-foreground flex items-center gap-1">
                      <Calendar className="h-3 w-3" />
                      {safeFormatDate(visit.visit_date, "MMMM d, yyyy")}
                    </div>
                  </div>
                </div>
              ...
              {visit.follow_up_date && (
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/40 rounded-lg px-3 py-2">
                  ...
                  <span>Follow-up: {safeFormatDate(visit.follow_up_date, "MMM d, yyyy")}</span>
                </div>
              )}
              ...
              {visit.notes && (
                <div className="rounded-lg bg-muted/40 p-3">
                  ...
                  <p className="text-sm text-foreground leading-relaxed">{displayNotes}</p>
                  {hasLongNotes && (
                    <button
                      type="button"
                      className="mt-2 text-xs text-primary font-medium hover:underline"
                      onClick={() =>
                        setExpandedVisitNotes((prev) => ({
                          ...prev,
                          [visit.id]: !isExpanded,
                        }))
                      }
