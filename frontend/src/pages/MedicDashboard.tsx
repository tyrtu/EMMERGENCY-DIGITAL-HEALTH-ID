...
interface PatientRecordLite {
  ...
  emergencyInfo?: {
    primaryEmergencyContacts?: Array<{ name?: string; relation?: string; phone?: string }>;
...
export default function MedicDashboard() {
  ...
  useEffect(() => {
    const loadMedicProfile = async () => {
      ...
      try {
        ...
        // Fallback draft below keeps UI usable even if profile fetch fails.
      } finally {
  ...
  const handleScanResult = (data: QRData) => {
    ...
    if (isMultiCasualtyMode) {
      ...
      if (duplicateIndex >= 0) {
        ...
        setIncidentAlert({
          message: `${data.name || "Patient"} is already in the casualty list.`,
          severity: "warning",
        });
        return;
      }
      ...
      setIncidentAlert({
        message: `${data.name || incomingId} added to multi-casualty triage.`,
        severity: "success",
      });
  ...
  const casualtyStats = useMemo(() => {
    const critical = casualtyList.filter((entry) => entry.triage === "critical").length;
    const caution = casualtyList.filter((entry) => entry.triage === "caution").length;
    const normal = casualtyList.filter((entry) => entry.triage === "normal").length;
  ...
  const scansToDisplay = searchQuery.trim()
    ? filteredScans
                ...
                <div className="mt-4 space-y-2">
                  <SheetClose asChild>
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                        activeView === "scans" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
                  ...
                  <SheetClose asChild>
                    <button
                      onClick={() => setActiveView("analytics")}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                        activeView === "analytics" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
                  ...
                  <SheetClose asChild>
                    <button
                      onClick={() => setActiveView("search")}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                        activeView === "search" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
                  ...
                  <SheetClose asChild>
                    <button
                      onClick={() => setActiveView("casualty")}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                        activeView === "casualty" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
                      <Users className="h-4 w-4" /> Multi-Casualty
                    </button>
                  </SheetClose>
                  <SheetClose asChild>
                    <button
                      onClick={() => setActiveView("profile")}
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium ${
                        activeView === "profile" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50"
                      }`}
                    >
                      <Stethoscope className="h-4 w-4" /> My Profile
                    </button>
                  </SheetClose>
                  <SheetClose asChild>
                    <button
              ...
              {!searchQuery && filteredScans.length > scansToDisplay.length && (
                <div className="pt-2 flex justify-center">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => setVisibleCount((prev) => prev + LATEST_SCAN_LIMIT)}
                  >
                ...
                <div className="card-medical space-y-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <Input value={patientDraft.bloodGroup} onChange={(e) => updatePatientDraft("bloodGroup", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Contact Phone</p>
                      <Input value={patientDraft.contactPhone} onChange={(e) => updatePatientDraft("contactPhone", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Contact Email</p>
                      <Input value={patientDraft.contactEmail} onChange={(e) => updatePatientDraft("contactEmail", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">City</p>
                      <Input value={patientDraft.city} onChange={(e) => updatePatientDraft("city", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Country</p>
                      <Input value={patientDraft.country} onChange={(e) => updatePatientDraft("country", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Preferred Hospital Name</p>
                      <Input value={patientDraft.preferredHospitalName} onChange={(e) => updatePatientDraft("preferredHospitalName", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Preferred Hospital Phone</p>
                      <Input value={patientDraft.preferredHospitalPhone} onChange={(e) => updatePatientDraft("preferredHospitalPhone", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Insurance Provider</p>
                      <Input value={patientDraft.insuranceProvider} onChange={(e) => updatePatientDraft("insuranceProvider", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Policy Number</p>
                      <Input value={patientDraft.insurancePolicyNumber} onChange={(e) => updatePatientDraft("insurancePolicyNumber", e.target.value)} />
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="space-y-1.5">
                      ...
                      <Input value={patientDraft.emergencyContactName} onChange={(e) => updatePatientDraft("emergencyContactName", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Relationship</p>
                      <Input value={patientDraft.emergencyContactRelation} onChange={(e) => updatePatientDraft("emergencyContactRelation", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Emergency Contact Phone</p>
                      <Input value={patientDraft.emergencyContactPhone} onChange={(e) => updatePatientDraft("emergencyContactPhone", e.target.value)} />
                    </div>
                  </div>

                  <div className="grid gap-3 md:grid-cols-3">
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Critical Allergies (line or comma separated)</p>
                      <Textarea value={patientDraft.allergiesText} onChange={(e) => updatePatientDraft("allergiesText", e.target.value)} rows={5} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Critical Conditions (line or comma separated)</p>
                      <Textarea value={patientDraft.conditionsText} onChange={(e) => updatePatientDraft("conditionsText", e.target.value)} rows={5} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Current Medications (line or comma separated)</p>
                      <Textarea value={patientDraft.medicationsText} onChange={(e) => updatePatientDraft("medicationsText", e.target.value)} rows={5} />
                    </div>
                  </div>
                ...
                <>
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      ...
                      <Input value={medicProfileDraft.name} onChange={(e) => updateMedicDraft("name", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Email</p>
                      <Input value={medicProfileDraft.email} onChange={(e) => updateMedicDraft("email", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Phone</p>
                      <Input value={medicProfileDraft.phone} onChange={(e) => updateMedicDraft("phone", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Specialization</p>
                      <Input value={medicProfileDraft.specialization} onChange={(e) => updateMedicDraft("specialization", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">License Number</p>
                      <Input value={medicProfileDraft.licenseNumber} onChange={(e) => updateMedicDraft("licenseNumber", e.target.value)} />
                    </div>
                    <div className="space-y-1.5">
                      <p className="text-xs text-muted-foreground">Auth ID</p>
                      <Input value={medicProfileDraft.authId} onChange={(e) => updateMedicDraft("authId", e.target.value)} className="font-mono" />
                    </div>
                  </div>
                      ...
                      <div className="flex items-center justify-between gap-3 flex-wrap">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => setScannedPatient(selectedCasualty)}
                          >
                            Open Full Editor
                          </Button>
                          <Button
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="rounded-lg border border-border p-3">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {selectedCasualty.allergies.length > 0 ? selectedCasualty.allergies.map((item) => (
                        ...
                        <div className="rounded-lg border border-border p-3">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {selectedCasualty.conditions.length > 0 ? selectedCasualty.conditions.map((item) => (
